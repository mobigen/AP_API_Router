...
class ApiRoute:
    ...
    def set_route(self) -> None:
        ...
        config.api_server_info, _ = db.select("SELECT * FROM tb_api_server_info")
        ...
        for api in config.api_info:
            ...
            self.router.add_api_route(
                ...
                self.route_api,
                methods=method,
                tags=[f'Route Category ({api["ctgry"]})'],
            )

        for api_name, conf_api_info in config.api_config.items():
            module_path = (
                f'{config.root_path}/ApiList/{conf_api_info["sub_dir"]}/{api_name}.py'
            )
            module_name = "api"
            ...
            self.router.add_api_route(
                f'{conf_api_info["url_prefix"]}/{conf_api_info["sub_dir"]}/{api_name}',
                module.api,
